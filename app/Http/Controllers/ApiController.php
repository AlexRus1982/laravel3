...
class ApiController extends Controller
{
    ...
    public function BasketGetCount(Request $request) {
        ...
        foreach ($products as $product) {
            ...
            $sumPrice += (int)$product->{"Цена"} * (int)$product->quantity;
        }
    ...
    public function BasketSetQuantity(Request $request) {
        $order_hash = $request->order_hash;
        $itemId = $request->itemId;
        $products = DB::table('basket_products')->where([['order_hash', '=', "{$order_hash}"], ['product_id', '=', "{$itemId}"],])->get();
        if (count($products) > 0){
    ...
    public function BasketList(Request $request) {
        ...
        $products = DB::table('basket_products')
                    ...
                    ->join('catalog', 'catalog.id', '=', 'basket_products.product_id')
                    ->select('basket_products.product_id', 'catalog.Наименование', 'catalog.Описание', 'catalog.Фото товара', 'catalog.Цена', 'catalog.URL адрес', 'basket_products.quantity', 'basket_products.category')
                    ->get();
        return $products;
    }
